...
require_once('../modelo/DB_registros.php');
...
$mostrarIncidencias = DB_contacto::obtieneIncidencias();
...
if(isset($_POST['enviar'])){
    ...
    if(!$existe){
        //Si no existe ningun incidencia lo añadimos
...
if(isset($_GET['id'])){
    $id_contacto=$_GET['id'];
    $incidencia= DB_contacto::obtieneCliente($id_contacto);
    asignarIdContacto($incidencia);
}

function comprueba($msjTxt){
    if($msjTxt!=null){
        header("Location: gestion-incidencia.php?state=si");
    }else{
        header("Location: gestion-incidencia.php?state=no");
    }
}
...
function asignarIdContacto($incidencia){
    global $smarty;
...
$smarty->display("gestion-incidencia.tpl");
?>
